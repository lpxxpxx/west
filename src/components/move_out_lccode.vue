<template>
  <div class="container adjustment">
    <tab :line-width=1>
      <tab-item :selected="index === 0" @on-item-click="changeIndex(0)">{{$t('accordingToTheLocationToCheck')}} - {{$t('moveOut')}}</tab-item>
    </tab>
    <div class="tab-swiper" v-show="index === 0">
      <div class="search">
        <span class="label">{{$t('moveOutLcCode')}}</span>
        <span class="underline name" title="">{{lcCode}}</span>
      </div>
      <div class="search search-plus">
        <scan-input :placeholder="$t('scanTheBarcodeOfStorageLocationHere')" :name="$t('targetLocation')" v-model="toLcCode"></scan-input>
      </div>
      <div class="table lcCode">
        <x-table full-bordered>
          ...
          <tbody>
            ...
            <tr v-if="lcCodeData.length === 0">
              <td colspan="3">{{$t('noMatchingRecordsWereFound')}}</td>
            </tr>
          </tbody>
        </x-table>
      </div>
      <div class="button" v-show="toLcCode">
        <flexbox>
          <flexbox-item>
...
export default {
  ...
  methods: {
    ...
    submit (type) {
      const submitData = []
      let reg = /^[1-9]\d{0,4}$/
      if (this.toLcCode === this.lcCode) {
        this.$vux.toast.show({
          type: 'text',
          text: this.$t('cannotBeEqual')
        })
        return false
      }
      for (let i = 0; i < this[`${type}Data`].length; i++) {
        ...
        if (qty && (!reg.test(Number(qty)) || Number(qty) > Number(this[`${type}Data`][i].piSellable))) {
          ...
          submitData.push({
            lcCode: this.lcCode,
            productBarcode: this[`${type}Data`][i].productBarcode,
            lcCodeNew: this.toLcCode,
            quantity: qty
          })
        }
      }
